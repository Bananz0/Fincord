...
/*
 * Ported from Gramophone (FoedusProgramme/Gramophone, GPL-3.0) on 2026-09-02; the original
 * copyright above is retained.
 *
 * One change from upstream. Gramophone builds against a media3 fork (nift4/media) whose
 * `androidx.media3.extractor.metadata.ReplayGainInfo` carries the ReplayGain values LAME writes
 * into an MP3 Info tag. That class has since been upstreamed into media3 proper as
 * `androidx.media3.extractor.mp3.Mp3InfoReplayGain` with the same shape - a peak plus two gain
 * fields of name/originator/gain - so this is a type swap and nothing is lost: the LAME path still
 * works, on stock media3.
 */
